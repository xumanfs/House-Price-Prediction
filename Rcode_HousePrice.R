...
train_set_3 <- dataset_3[!is.na(dataset_3$SalePrice), ]

correlation_matrix <- cor(train_set_3[, index_numeric])
variable_names_sorted_by_corr <- names(sort(abs(correlation_matrix[,"SalePrice"]), decreasing = TRUE))
variable_names_sorted_by_corr <- variable_names_sorted_by_corr[-which(variable_names_sorted_by_corr == "Id")]
correlation_matrix <- correlation_matrix[variable_names_sorted_by_corr, variable_names_sorted_by_corr]

corrplot(correlation_matrix)

### 2.3.3 Classification variables

index_factor <- which(sapply(dataset_3, is.factor))
factor_variables <- dataset_3[c(which(names(dataset_3) == "SalePrice"), index_factor)]

p_aov <- apply(factor_variables, 2, function(x) {
  fit_aov <- aov(log(SalePrice) ~ x, data = dataset_3)
...
## 3.2 Knn
dataset_knn <- dataset_4[-75]
dataset_dummy <- as.data.frame(model.matrix(~.-1, dataset_knn))
x_train_dummy <- dataset_dummy[dataset_dummy$Id %in% train_set$Id, ]
x_test_dummy <- dataset_dummy[dataset_dummy$Id %in% test_set$Id, ]

fit_knn <- knn.reg(train = x_train_dummy, test = x_test_dummy, y = train_set_4$SalePrice, k = 1)
y_predict_knn <- fit_knn[["pred"]]

sub_knn <- data.frame(Id = test_set$Id, SalePrice = y_predict_knn)
write.csv(sub_knn, file = "sub_knn.csv", row.names = FALSE)
...
## 3.5 average the lasso and gbdt model

y_predict_lasso_gbdt <- (y_hat_lasso + y_hat_gbdt)/2
sub_lasso_gbdt <- data.frame(Id = test_set$Id, SalePrice = y_predict_lasso_gbdt)
write.csv(sub_lasso_gbdt, file = "sub_lasso_gbdt.csv", row.names = FALSE)

## 3.6 average the random forest, lasso and gbdt model
